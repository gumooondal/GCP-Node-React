...
const Header = ({ onLogout, isLoggedIn }) => {
  ...
  const handleLogoutClick = async () => {
    try {
      ...
      // 로그아웃 성공 메시지 확인
      if (response.status === 200 && response.data.message === '로그아웃 성공') {
  ...
  return (
    <header className="app-header">
      ...
      {isMenuOpen && (
        <div className="menu">
          <ul> {/* <ul>로 감싸줍니다. */}
            <li><div className="login-buttons-container">
              {isLoggedIn ? (
                <button className="logout-button" onClick={handleLogoutClick}>
                  로그아웃
                </button>
              ) : (
                <button className="nonlogin-button">
                  비로그인
                </button>
              )}
            </div>
            ...
            <li>문의사항 - &nbsp;
              ...
              <svg
                onClick={() => window.open('https://www.instagram.com/development_climber/?igsh=cnBjcHNkcnJ5azE4&utm_source=qr', '_blank')}
